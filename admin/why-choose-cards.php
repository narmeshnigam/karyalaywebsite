<?php
/**
 * Admin Why Choose Cards List Page
 */
...
$cards = $cardModel->getAll($filters, $per_page, $offset);
$total_cards = $cardModel->count($filters);
$total_pages = ceil($total_cards / $per_page);

include_admin_header('Why Choose Cards');
?>

<div class="admin-page-header">
    <div class="admin-page-header-content">
        <h1 class="admin-page-title">Why Choose Cards</h1>
...
<div class="admin-card">
    <?php if (empty($cards)): ?>
        ...
        render_empty_state(
            ...
            '/admin/why-choose-cards/new.php',
            'Add Card'
        );
        ?>
    <?php else: ?>
                ...
                <tbody>
                    <?php foreach ($cards as $card): ?>
                        <tr>
                            ...
                            <td>
                                <div class="table-cell-primary"><?php echo htmlspecialchars($card['title']); ?></div>
                                <?php if (!empty($card['description'])): ?>
                                    <div class="table-cell-secondary">
                                        <?php echo htmlspecialchars(substr($card['description'], 0, 60)); ?>
                                        <?php echo strlen($card['description']) > 60 ? '...' : ''; ?>
                                    </div>
                                <?php endif; ?>
                            </td>
                            <td>
                                <?php if (!empty($card['link_url'])): ?>
                                    <a href="<?php echo htmlspecialchars($card['link_url']); ?>" target="_blank" class="link-preview">View Link</a>
                                <?php else: ?>
                                    <span class="text-muted">No link</span>
                                <?php endif; ?>
                            </td>
                            <td><?php echo get_status_badge($card['status']); ?></td>
                            <td><?php echo htmlspecialchars($card['display_order']); ?></td>
        ...
        <?php if ($total_pages > 1): ?>
            <div class="admin-card-footer">
                <?php 
                $base_url = '/admin/why-choose-cards.php';
                $query_params = [];
        ...
        <div class="admin-card-footer">
            <p class="admin-card-footer-text">
                Showing <?php echo count($cards); ?> of <?php echo $total_cards; ?> card<?php echo $total_cards !== 1 ? 's' : ''; ?>
            </p>
        </div>
    <?php endif; ?>
</div>
...
.admin-filter-input, .admin-filter-select { padding: var(--spacing-2) var(--spacing-3); border: 1px solid var(--color-gray-300); border-radius: var(--radius-md); font-size: var(--font-size-base); color: var(--color-gray-900); }
.admin-filter-input:focus, .admin-filter-select:focus { outline: none; border-color: var(--color-primary); box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1); }
...
.table-cell-secondary { font-size: var(--font-size-sm); color: var(--color-gray-600); margin-top: var(--spacing-1); }
.card-preview-thumb { width: 100px; height: 56px; object-fit: cover; border-radius: var(--radius-md); border: 1px solid var(--color-gray-200); }
